...
namespace SignaturePad {
    public class SignaturePadView : RelativeLayout
    {
        ...
        // Return bytes array of currently drawn signature
        public Byte[] GetBytes(bool shouldCrop = true, bool keepAspectRatio = true)
        {
            var tmpImage = GetImage(strokeColor, Color.Transparent, new System.Drawing.SizeF(Width, Height), 1, shouldCrop, keepAspectRatio);
            if (tmpImage != null)
            {
                System.IO.MemoryStream stream = new System.IO.MemoryStream();
                tmpImage.Compress(Bitmap.CompressFormat.Png, 100, stream);
                if (stream.Length > 0)
                {
        ...
        // Load bytes array to draw a signature in the view
        public void LoadBytes(Byte[] pByteArray)
        {
            if (pByteArray != null && pByteArray.Length == 0)
                return;

            using (Bitmap bitmap = BitmapFactory.DecodeByteArray(pByteArray, 0, pByteArray.Length))
            {
                if (bitmap != null)
                {
                    UseBitmapBuffer = true;
